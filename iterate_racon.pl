...
sub version {
	print qq/
iterate_racon: automate the process of polishing a nanopore consensus
(Version: $VERSION)
/;
	return 1;
}
...
sub usage {
  ...
  USAGE: $0 -c contigs.fa -i iterations -f forward_reads.fq -r reverse_reads.fq -s single_reads.fq

    Basic arguments

    -c  <fasta file>         fasta file of contigs and scaffolds
    -i  <integer>            number of iterations of polishing
                             default: 5

    Optional Arguments

    -o  <output name>        base name for output files (default: iterate_racon)
