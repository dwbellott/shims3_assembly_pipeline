...
sub main{
  ...
  ## Is $contigs a sequence file that we can read?
  if (defined($contigs)){
...
sub version {
	print qq/
minimap2_and_gap5: automate the process of aligning reads to an assembly and exporting to gap5
(Version: $VERSION)
/;
	return 1;
}
...
sub usage {
  ...
  USAGE: $0 -c contigs.fa -f forward_reads.fq -r reverse_reads.fq -s single_reads.fq
    ...
    -o  <output name>        base name for output files (default: mm2ag)
