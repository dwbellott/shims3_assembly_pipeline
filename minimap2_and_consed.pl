...
sub main{
  ...
  ## Is there a contig sequence file that we can read?
  if (defined($contigs)){
    if (-e $contigs && -f _ && -r _){
    }else{
      $help = 1;
      $warning .= "Cannot read sequence file: $contigs\n";
    }
  }else{
    $help = 1;
    $warning .= "No contig sequence file specified!\n";
  }
...
sub version {
	print qq/
minimap2_and_consed: automate the process of aligning reads to an assembly and exporting to consed
(Version: $VERSION)
/;
	return 1;
}
...
sub usage {
  ...
  USAGE: $0 -c contigs.fa -f forward_reads.fq -r reverse_reads.fq -s single_reads.fq

    Basic arguments

    -c  <fasta file>         fasta file of contigs and scaffolds

    Optional Arguments

    -o  <output name>        base name for output files (default: mm2ac)
    -d  <outpur directory>   directory for output  (default: current directory)

    Illumina Reads

    -f  <fastq files(s)>     fastq files of reads; may be supplied multiple times
    -r  <fastq file(s)>      or once as a comma separated list of fastq files
    -s  <fastq file(s)>
