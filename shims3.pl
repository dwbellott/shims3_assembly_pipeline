...
sub main{
  ...
  ## set default iterations for racon
  ...
  @freads = split(/\,/,(join(",",@freads)));
  @rreads = split(/\,/,(join(",",@rreads)));
  @sreads = split(/\,/,(join(",",@sreads)));
  @preads = split(/\,/,(join(",",@preads)));
  ...
  ## Check that any forward and reverse reads are paired
  if (@freads || @rreads){
    unless (@freads == @rreads){
      $help = 1;
      $warning .= "Unequal number of forward and reverse read files\n";
    }
  }
  ...
  if (defined($directory)){
		if (-e $directory){
			...
			$warning .= "creating directory $directory\n";
			mkdir abs_path($directory);
		}
    $stem .= "$directory/";
	}else{
    $directory = getcwd;
  ...
  ## Check on the base name for the output
  if (defined($output)){
  }else{
    $output = "shims3";
  }
  ...
  ## racon
  $racon_exec = which('racon');
  unless (defined($racon_exec) && -e $racon_exec && -x _){
    $help = 1;
    $warning .= "Please install racon: https://github.com/lbcb-sci/racon\n";
  }

  ## samtools
  $samtools_exec = which('samtools');
  unless (defined($samtools_exec) && -e $samtools_exec && -x _){
    $help = 1;
    $warning .= "Please install samtools: http://www.htslib.org\n";
  }

  ## staden package if necessary

  if ($gap5){

    ### tg_index
    $tg_index_exec = which('tg_index');
    unless (defined($tg_index_exec) && -e $tg_index_exec && -x _){
      $help = 1;
      $warning .= "Please install the Staden Package to use tg_index and gap5: http://staden.sourceforge.net\n";
    }

  }


  ## consed package if necessary

  if ($consed){

    ### consed
    $consed_exec = which('consed');
    unless (defined($consed_exec) && -e $consed_exec && -x _){
      $help = 1;
      $warning .= "Please install consed: http://www.phrap.org/consed/consed.html\n";
    }

    ### makeRegionsFile.perl
    $makeregionsfile_exec = which('makeRegionsFile.perl');
    unless (defined($makeregionsfile_exec) && -e $makeregionsfile_exec && -x _){
      $help = 1;
      $warning .= "Please install consed to use makeRegionsFile.perl: http://www.phrap.org/consed/consed.html\n";
    }
  ...
  ## Set up files:
  $fl_reads = "$stem.fl.fq.gz";
  $longest_read = "$stem.longest.fl.fa";
  $longest_paf_one = "$stem.longest.fl.paf";
  $longest_intermediate = "$stem.longest.fl.racon.fa";
  $longest_paf_two = "$stem.longest.fl.racon.paf";
  $longest_consensus = "$stem.fl.consensus.fa";

  ## Extract full-length reads from nanopore read files
  &extract_full_length();

  ## Make a consensus from the full-length reads
  &make_fl_consensus();

  ## Polish this consensus with all available data
  my $polished = &iterate_racon;
  print STDERR "Polished contigs are in $polished\n";
  ...
  if (@sreads){
    push(@gap5sams, &align_single($polished));
  }
  if (@freads){
    push(@gap5sams, &align_paired($polished));
  }
  ...
  if ($gap5){
    if (@preads){
      push(@gap5sams, &align_pacbio($polished));
    }
    if (@nreads){
      push(@gap5sams, &align_nanopore($polished));
    }
    &make_gap5_database(\@gap5sams);
  }

  if ($consed){
    my $subread_length = 1000;
    if (@preads){
      push(@consedsams, &align_pacbio_for_consed($polished, $subread_length));
    }
    if (@nreads){
      push(@consedsams, &align_nanopore_for_consed($polished, $subread_length));
    }
    &make_ace_file($polished, \@consedsams);
  }

  # Make sure other users can read and write

  chmod_recursive( 0775, $directory);
...
USAGE: $0 -b vector.fa -n nanopore_reads.fq
